...
class CrossAttention(nn.Module):
    ...
    def forward(self, q, v): #normal CA
        A = self.a1(v)  # s2c1
        B = self.b1(v)
...
class transNet_norm_q(nn.Module):
  def __init__(self, nfeats=64):
    super(transNet_norm_q, self).__init__()
    ...
    self.conv3_1 = nn.Sequential(
        nn.Conv2d(128, nfeats, kernel_size=3, stride=1, padding=1),
        nn.LeakyReLU(negative_slope=0.2)
    )
  ...
  def forward(self, ir, vi, ir_rever, vi_rever):
  # def forward(self, ir, vi, ir_rever, vi_rever, fuse_feats_ori, orib_feat_edge, fuse_feats_rever, reverb_feat_edge):

    # head
    irb = torch.cat((ir, ir_rever), dim=1)
    visb = torch.cat((vi, vi_rever), dim=1)
    irb_feat = self.conv1_1(irb)
    visb_feat = self.conv2_1(visb)

    # body-densenet
    irb_dfeats = self.orib_path(irb_feat)
    visb_dfeats = self.reverb_path(visb_feat)
    # irb_dfeats_enhan = self.CA(visb_dfeats, irb_dfeats)
    irb_dfeats_enhan = self.CA(irb_dfeats, visb_dfeats)
  # orib_dfeats_enhan = self.CA_WD(reverb_dfeats, orib_dfeats)
  #   visb_dfeats_enhan = self.CA(irb_dfeats, visb_dfeats)
    visb_dfeats_enhan = self.CA(visb_dfeats, irb_dfeats)

    return irb_dfeats, visb_dfeats, irb_dfeats_enhan, visb_dfeats_enhan
